...
/**
 * Layout Templates for Gallery Display
 * These are the available layout presets that clients can choose for their galleries.
 * Each layout combines a header style, grid pattern, aspect ratio, and color theme.
 */
 ...
 */
export function getLayoutTemplate(layoutId: string): LayoutTemplate | undefined {
    return layoutTemplates.find(template => template.id === layoutId);
}
